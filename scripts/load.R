...
# Load datasets
cares_raw <- read_csv(here(raw_data_path, 'unicef-data-20250727.csv'))
pop_raw <- read_xlsx(here(raw_data_path, 'WPP2022_GEN_F01_DEMOGRAPHIC_INDICATORS_COMPACT_REV1.xlsx'),
                     sheet = "Projections", skip = 16,
                     col_types = 'text') # Avoid wrong types
mortality <- read_xlsx(here(raw_data_path, 'On-track and off-track countries.xlsx'))
...
merged <- cares %>%
  ...
  drop_na() # Take care of the observations with missing values 
